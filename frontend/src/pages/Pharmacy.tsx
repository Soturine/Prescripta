      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(20rem,.72fr)_minmax(0,1.28fr)]">
        {can("pharmacy.intervention.write") ? (
          <form className="surface-card grid content-start gap-3 p-5" onSubmit={(event) => { event.preventDefault(); createMutation.mutate(); }}>
            ...
            <input aria-label={t("pharmacy.patientId")} className="field" min={1} onChange={(event) => setForm({ ...form, patientId: event.target.value })} placeholder={t("pharmacy.patientId")} type="number" value={form.patientId} />
            <input aria-label={t("pharmacy.medicationId")} className="field" min={1} onChange={(event) => setForm({ ...form, medicationId: event.target.value })} placeholder={t("pharmacy.medicationId")} type="number" value={form.medicationId} />
          ...
          {query.data?.map((item) => (
            <article className="surface-card overflow-hidden" key={item.id}>
              <div className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-100 bg-slate-50/70 p-5">
                <div className="flex gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-amber-50 text-amber-800"><MessageSquareWarning aria-hidden="true" className="h-5 w-5" /></span><div><h3 className="font-black">#{item.id} · {humanizeTechnicalValue(item.intervention_type)}</h3><p className="mt-1 text-xs text-slate-500">{t("pharmacy.version", { version: item.version })}</p></div></div>
              ...
              <div className="grid gap-5 p-5 lg:grid-cols-[minmax(0,1fr)_15rem]">
                <div>
                  <div className="flex flex-wrap gap-2 text-xs font-bold text-slate-600"><span className="inline-flex items-center gap-1 rounded-full bg-cyan-50 px-2.5 py-1"><UserRound aria-hidden="true" className="h-3.5 w-3.5" />{t("pharmacy.patient", { id: item.patient_id })}</span><span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2.5 py-1"><Pill aria-hidden="true" className="h-3.5 w-3.5" />{item.medication_id ? t("pharmacy.medication", { id: item.medication_id }) : t("pharmacy.noMedication")}</span></div>
