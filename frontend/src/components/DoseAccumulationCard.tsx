...
export default function DoseAccumulationCard({ summary }: { summary: DoseSummary }) {
  const rows = [
    ...
    ["Duração planejada", summary.duration_days ? `${summary.duration_days} dias` : "Não informada"],
    [
    ...
    ["Duração máxima", summary.max_duration_days ? `${summary.max_duration_days} dias` : "-"],
    [
      "Limite acumulado",
      summary.max_cumulative_dose_mg ? formatDose(summary.max_cumulative_dose_mg) : "-",
    ],
    ["Uso contínuo", summary.continuous_use ? "Sim" : "Não"],
    ["Monitoramento necessário", summary.monitoring_required ? "Sim" : "Não"],
  ];
  const mechanismRows = [
    ["Mecanismo", summary.mechanism_profile.mechanism_of_action ?? "-"],
    ["Metabolização", summary.mechanism_profile.metabolism_organs.join(", ") || "-"],
    ["Eliminação", summary.mechanism_profile.elimination_organs.join(", ") || "-"],
    ["Eliminação renal", summary.mechanism_profile.renal_elimination_level],
    ["Metabolismo hepático", summary.mechanism_profile.hepatic_metabolism_level],
    ["CYP/interações", summary.mechanism_profile.cyp_interactions.join(", ") || "-"],
  ];
    ...
    <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-4 border-t border-slate-100 pt-4">
        <h3 className="text-sm font-bold text-ink">
          Perfil farmacocinético/farmacodinâmico
        </h3>
