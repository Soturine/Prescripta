...
test.describe("Research, RWE e workflows profissionais", () => {
  test("pesquisador vê somente agregados, attrition, provenance e DQ sintéticos", async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: "Research & RWE", exact: true })).toBeVisible();
    await expect(page.getByRole("link", { name: "Pacientes", exact: true })).toHaveCount(0);
    await page.getByRole("link", { name: "Research & RWE", exact: true }).click();

    await expect(page.getByRole("heading", { name: "Research & RWE" })).toBeVisible();
    await expect(
  ...
  test("farmacêutico visualiza intervenção explícita sem alteração automática", async ({ page }) => {
    ...
    await page.getByRole("link", { name: "Farmácia", exact: true }).click();
    await expect(page.getByRole("heading", { name: "Pharmacy workflow" })).toBeVisible();
